// journeyman.ts — the only place Journeyman actually writes to the Journeyman contract.
//
// Writes go through the Journeyman Agent Wallet's viem WalletClient (Circle MPC-backed,
...
/** Posts an open job on Journeyman (beneficiary = zero address = open for applications). Returns the escrowId. */
/**
 ...
 *
 * `setYieldOptIn` is on JourneymanYield, not on the escrow contract, and the
 ...
 * being deployed. Journeyman can execute the instruction, the way it executes an
 ...
 * by its owner, not Journeyman appointing itself.
 */
...
export async function createEscrow(
  ...
  // Approve Journeyman to pull `deposit` (totalAmount + platform fee, in USDC's own
  ...
  const approveHash = await signer.walletClient.writeContract({
    ...
    args: [config.journeymanAddress, deposit],
  });
  ...
  const created = receipt.logs.find((log) => log.address.toLowerCase() === config.journeymanAddress.toLowerCase());
  if (!created) throw new Error(`createEscrow tx ${hash} mined but no Journeyman log found`);
...
/**
 * Every Journeyman write, funnelled through one place.
 *
 * `as` is who signs. It defaults to the Journeyman treasury, which is what every
 * call site did implicitly before — Journeyman's own actions (hiring, approving,
 * rejecting, escalating) are unchanged and still go out as Journeyman.
 *
 ...
 * Journeyman authorises those on `msg.sender`. Journeyman cannot apply on someone's
 * behalf from its own wallet — the contract would record Journeyman as the
...
// ── Journeyman's own actions (signed by the treasury) ───────────────────────────
...
/** Human-arbiter escalation path — Journeyman's one-way key can never do this itself; it only calls it after max revisions. */
...
// Journeyman authorises each of these on msg.sender, so the signer here is the
// freelancer, never Journeyman. In managed mode that wallet is a Circle MPC wallet
// Journeyman provisioned for them; in bring-your-own mode these never run at all
// because the freelancer signs from their own wallet via Journeyman's dApp.
 ...
 * makes Journeyman's reputation real rather than derived: humans and clients rating
...
//
// This matters beyond bookkeeping. Journeyman's central claim is that no machine in
 ...
 * claim on the escrow is exactly what makes Journeyman trustworthy, and the
 ...
 *
 * Journeyman emits the answer directly:
 *
  ...
  for (let i = 0; i < chunks; i++) {
    ...
    try {
      logs = await pc.getLogs({
        address: config.journeymanAddress as `0x${string}`,
 ...
 * route to resolution at all. Journeyman built this for exactly that case.
 *
