 ...
 * applicants and release payment — surfaced inside Journeyman. This module is the
 * seam.
 *
 * The daemon is NOT being rewritten into Journeyman's Express backend. It works,
 * it runs unattended, and a rewrite would be a lot of new code that produces
 * exactly the behaviour we already have. So Journeyman talks to two backends: the
 * Express API for Journeyman's own concerns, and this one for agent activity.
 *
...
/** Whether Autopilot is reachable at all. False in a Journeyman-only deploy. */
 ...
 * and calling in Journeyman's dispute system. Everything downstream of it is a
 ...
 *
 * This is how Journeyman knows a job is on Autopilot: the chain does not record it
...
export const AUTOPILOT_BRIEF_KEY = "journeyman:autopilot-brief";
