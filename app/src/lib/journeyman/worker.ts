 ...
 *
 * Journeyman's demand side was always easy: an agent makes one HTTP call and x402
...
const SESSION_KEY = "journeyman:worker-id";
 ...
 */
const WORKER_ADDRESS_KEY = "journeyman.worker.address";

/** Fires when the signed-in worker changes, so React can re-read it. */
export const WORKER_IDENTITY_EVENT = "journeyman:worker-identity";
 ...
 * wallet is the depositor, so the escrow answers to them — not to Journeyman and
