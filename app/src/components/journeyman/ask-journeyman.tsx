/**
 * ASK JOURNEYMAN — the question box, on every page.
 *
 * WHY IT EXISTS
 *
 * Journeyman's central claim is that neither side has to trust the other, and that
...
export function AskJourneyman({ viewer }: { viewer?: AskViewer }) {
  ...
  const send = useCallback(
    async (question: string) => {
      ...
      try {
        const answer = await askJourneyman(next, {
          ...viewer,
        ...
        setError(
          e instanceof AssistantBusy
            ? e.message
            : "I could not reach the assistant just now. Everything else on Journeyman works normally.",
        );
      } finally {
        setThinking(false);
      }
  ...
  return (
    <>
            ...
            <header className="flex items-center justify-between gap-2 px-5 py-4 border-b shrink-0 rounded-t-2xl bg-gradient-to-b from-primary/[0.07] to-transparent">
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  <div className="font-display font-semibold leading-none">Ask Journeyman</div>
              ...
              {turns.length === 0 && (
                <div className="space-y-3">
                  <p className="text-sm text-muted-foreground leading-relaxed">
                    Ask me anything about how Journeyman works — escrow, Autopilot,
      ...
      <motion.button
        type="button"
        onClick={() => setOpen((o) => !o)}
        aria-label={open ? "Close Journeyman assistant" : "Ask Journeyman a question"}
        aria-expanded={open}
