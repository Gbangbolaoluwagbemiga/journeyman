...
vi.mock("@/lib/journeyman/agent-api", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@/lib/journeyman/agent-api")>();
  return actual;
});
...
describe("fetchDecisionsForEscrow", () => {
  it("returns only the decisions belonging to that escrow's task", async () => {
    const { fetchDecisionsForEscrow } = await import("@/lib/journeyman/agent-api");
