 ...
 * escaped, and that it never implies it can act — Journeyman's argument is that
 * nobody has to be trusted, and a box that offers to move money for you argues
 * the opposite.
 */

const askJourneyman = vi.fn();
class AssistantBusy extends Error {}

vi.mock("@/lib/journeyman/agent-api", () => ({
  AUTOPILOT_CONFIGURED: true,
  AssistantBusy,
  askJourneyman: (m: unknown, v: unknown) => askJourneyman(m, v),
}));

const { AskJourneyman } = await import("@/components/journeyman/ask-journeyman");

const show = (viewer?: unknown) =>
  render(
    <MemoryRouter initialEntries={["/jobs"]}>
      <AskJourneyman viewer={viewer as never} />
    </MemoryRouter>,
  );

beforeEach(() => {
  vi.clearAllMocks();
  askJourneyman.mockResolvedValue("The budget is locked in escrow from the moment the job is created.");
});
...
describe("opening it", () => {
  it("stays out of the way until asked for", () => {
    ...
    expect(screen.getByRole("button", { name: /ask journeyman/i })).toBeInTheDocument();
  });

  it("offers the questions people are actually nervous about", async () => {
    show();
    await userEvent.click(screen.getByRole("button", { name: /ask journeyman/i }));
...
describe("asking", () => {
  ...
  it("tells the assistant who is asking and from where", async () => {
    ...
    await waitFor(() => expect(askJourneyman).toHaveBeenCalled());
    expect(askJourneyman.mock.calls[0][1]).toMatchObject({
      role: "freelancer",
      working: 1,
      page: "Browse Jobs",
    });
  ...
  it("sends on Enter, and keeps Shift+Enter for a new line", async () => {
    ...
    expect(askJourneyman).not.toHaveBeenCalled();
...
describe("when it cannot answer", () => {
  it("says the rest of Journeyman is fine, because it is", async () => {
    // The assistant is a convenience. A failure here must not read as the
    // marketplace being down.
    askJourneyman.mockRejectedValue(new Error("offline"));

    show();
    await userEvent.click(screen.getByRole("button", { name: /ask journeyman/i }));
    await userEvent.click(await screen.findByText(/take the money back/i));

    expect(await screen.findByText(/everything else on journeyman works normally/i)).toBeInTheDocument();
  });

  it("passes on the rate-limit message rather than burying it", async () => {
    askJourneyman.mockRejectedValue(new AssistantBusy("That is a lot of questions at once."));
