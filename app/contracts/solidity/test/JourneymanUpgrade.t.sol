 ...
 */
contract JourneymanV2 is Journeyman {
     ...
     *
     * This fixture inherits the whole of Journeyman, so it is always within a few
    ...
    function setFutureSetting(uint256 v) external {
        ...
        // than this fixture has: it inherits the whole of Journeyman and so sits
...
contract JourneymanBrokenV2 is Journeyman {
    function version() external pure override returns (string memory) {
        return "broken";
    }
}
 ...
 */
contract JourneymanUpgradeTest is JobManagerBase {
    /* ─────────────── Who may upgrade ─────────────── */

    function test_ownerCanUpgrade() public {
        JourneymanV2 v2 = new JourneymanV2();
        sf.upgradeToAndCall(address(v2), "");
        assertEq(JourneymanV2(payable(address(sf))).version(), "2.1.0-future");
    }
    ...
    function test_liveEscrowSurvivesUpgradeIntact() public {
        ...
        sf.upgradeToAndCall(address(new JourneymanV2()), "");
    ...
    function test_milestoneDetailSurvivesUpgrade() public {
        ...
        Journeyman.Milestone[] memory before = sf.getMilestones(id);
        sf.upgradeToAndCall(address(new JourneymanV2()), "");
        Journeyman.Milestone[] memory afterUpgrade = sf.getMilestones(id);
    ...
    function test_v2CanAddAndUseNewState() public {
        ...
        JourneymanV2 v2 = JourneymanV2(payable(address(sf)));
    ...
    function test_canUpgradeRepeatedly() public {
        ...
        sf.upgradeToAndCall(address(new JourneymanBrokenV2()), "");
