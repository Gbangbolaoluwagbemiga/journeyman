 ...
 *   - Journeyman's three client modes all end at the same contract state
 *
 ...
 * more or less.
 */
contract JourneymanE2ETest is JobManagerBase {
    /* ─────────────────────── helpers ─────────────────────── */
    ...
    /**
     * Row one of Journeyman's model: a human client who manages the job themselves.
     * Journeyman exactly as it works today, walked end to end.
     */
    function test_e2e_manualJob_postToPaidAndRated() public {
        ...
        assertEq(uint8(esc.status), uint8(Journeyman.EscrowStatus.Released), "escrow released");
    ...
    function test_e2e_autopilotJob_withRejectionAndRevision() public {
        ...
        Journeyman.Escrow memory esc = sf.getEscrow(id);
        assertEq(uint8(esc.status), uint8(Journeyman.EscrowStatus.Released));
    ...
    function test_e2e_unfilledJobCancelledAndRefunded() public {
        ...
        Journeyman.Escrow memory esc = sf.getEscrow(id);
        assertEq(uint8(esc.status), uint8(Journeyman.EscrowStatus.Cancelled));
    ...
    /**
     * Journeyman's whole point is that these coexist in one marketplace. Run all
