...
/** The slice of Journeyman this controller needs to reason about safety. */
interface IJourneymanEscrows {
 ...
 * smaller and worse. Two attempts at shaving bytes are recorded in Journeyman.sol
 ...
 */
contract JourneymanYield is IJourneymanYield, Ownable2Step, ReentrancyGuard {
    ...
    function setYieldOptIn(uint256 escrowId, bool optedIn) external {
        IJourneymanEscrows.Escrow memory esc = IJourneymanEscrows(escrow).getEscrow(escrowId);
        ...
        if (esc.workStarted || esc.status != IJourneymanEscrows.EscrowStatus.Pending) {
    ...
    function investableCeiling(uint256 escrowId) public view returns (uint256) {
        ...
        if (
            esc.status != IJourneymanEscrows.EscrowStatus.Pending &&
            esc.status != IJourneymanEscrows.EscrowStatus.InProgress
        ) return 0;
        ...
        IJourneymanEscrows.Milestone[] memory ms = IJourneymanEscrows(escrow).getMilestones(escrowId);
        for (uint256 i; i < ms.length; ++i) {
            if (ms[i].status == IJourneymanEscrows.MilestoneStatus.Approved) continue;
    ...
    function investIdle(uint256 escrowId) external nonReentrant {
        ...
        IJourneymanPull(escrow).releaseToYield(token, amount);
    ...
    function distributeYield(uint256 escrowId) external nonReentrant {
        ...
        if (
            esc.status == IJourneymanEscrows.EscrowStatus.Pending ||
            esc.status == IJourneymanEscrows.EscrowStatus.InProgress ||
            esc.status == IJourneymanEscrows.EscrowStatus.Disputed
        ...
        address platform = IJourneymanEscrows(escrow).feeCollector();
